...
\begin{tikzpicture}
  ...
  \node[latent, below=of x1, xshift = 1cm] (mu) {$\mu$};
...
Now, we delve more deeply into the math behind using such mixture models.  To start, the models involve $x$ (data), $z$ (latent variables), $\pi$ (prior of latent variable), $\{\mu_k\}$ (parameters for conditional distribution).  We will use $x$ and $z$ to denote all $n$ observations of the data and latent variables.
...
\begin{enumerate}
	...
	\item Compute the likelihood $\displaystyle p(x, z | \pi, \mu) = \sum_n \sum_k z_{nk} \log \pi_k + z_{nk} \log p(x_n |\mu_k)$.
...
The above is called the {\bf EM Algorithm}, where step 4 computes the actual expectation maximization, and step 5 actually maximizes through the MLE.  $\pi$ will be Categorical and $\mu$ will be the means of the Gaussians themselves here.
...
&\ge \sum_n \sum_k q(z_{nk}) \log \frac{p(x_n, z_n |\pi, \mu)}{q(z_n)} \\
&= \left[\sum_n \sum_k q(z_{nk}) \log p(x_n, z_n |\pi, \mu) \right]- \sum_n \left[\sum_k q(z_{nk}) \log q(z_n)\right] \\
...
\begin{align*}
\pi, \mu \gets \argmin_{\pi, \mu} \sum_n \sum_k q_{nk} \log \pi_k + q_{nk} \log p(x_n |  \mu_k),	
\end{align*}
